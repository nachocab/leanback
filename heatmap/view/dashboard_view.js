...
(function() {
  ...
  window.DashboardView = (function(_super) {
    ...
    DashboardView.prototype.showTitle = function() {
      ...
      title = "" + (this.getCurrentGenes().length) + " genes";
      if (this.model.get("showGroups")) {
        title += " from " + (joinSentence(this.model.columnGroups));
      }
      if (this.model.get("showSuffix")) {
        title += " " + (unescape(this.model.get("suffix")));
      }
      if (this.model.get("showClusters") && (currentCluster = this.model.get("currentCluster"))) {
        title += " (Cluster " + currentCluster + ")";
      }
      if (this.model.get("showTags") && (currentTag = this.model.get("currentTag"))) {
    ...
    DashboardView.prototype.getCurrentGenes = function() {
      var currentCluster, currentTag;
      if (this.model.get("showClusters") && (currentCluster = this.model.get("currentCluster"))) {
        return d3.selectAll(".row[cluster='" + currentCluster + "']")[0];
      } else if (this.model.get("showTags") && (currentTag = this.model.get("currentTag"))) {
        return d3.selectAll(".tag[name='" + currentTag + "']")[0];
      } else {
        return this.model.parsedGenes;
      }
    };
