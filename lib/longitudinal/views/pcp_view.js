...
(function() {
  ...
  window.PcpView = (function(_super) {
    ...
    PcpView.prototype.initialize = function() {
      var _this = this;
      if (!this.model.get("hidePCP")) {
        this.render();
        this.model.on("change:currentGeneName", function() {
          return _this.showCurrentGeneName();
        });
        this.model.on("change:clickedGeneName", function() {
          return _this.showClickedGeneName();
        });
    ...
    PcpView.prototype.render = function() {
      ...
      this.y = d3.scale.linear().domain(this.model.getExtent()).range([this.height, 0], .2);
      this.line = d3.svg.line().defined(function(d) {
        return _.isFinite(d.geneExpression);
      }).x(function(d) {
        return _this.x(d.condition);
      }).y(function(d) {
        return _this.y(d.geneExpression);
      });
      ...
      this.addGeneExpressionLines();
      return this.pcp.append("line").attr("y1", this.y(0)).attr("y2", this.y(0)).attr("x2", this.width).attr("class", "axis zero");
    };

    PcpView.prototype.addGeneExpressionLines = function() {
      var _this = this;
      this.pcp.selectAll(".expression-line").data(this.getGeneExpressions()).enter().append("path").attr("class", "expression-line").attr("name", function(d, i) {
        return _this.model.geneNames[i];
      }).attr("stroke", "steelblue").attr("stroke-opacity", .5).attr("d", this.line);
      if (this.model.get("showClusters")) {
    ...
    PcpView.prototype.getGeneExpressions = function() {
      if (this.model.get("showGroups")) {
        return this.model.groupedGeneExpressions;
      } else {
        return this.model.geneExpressions;
      }
    };

    PcpView.prototype.changeCurrentGeneName = function(e) {
      e.stopPropagation();
      return this.model.set({
        currentGeneName: e.type === "mouseover" ? d3.select(e.target).attr("name") : null
      });
    };

    PcpView.prototype.changeClickedGeneName = function(e) {
      e.stopPropagation();
      return this.model.set({
        clickedGeneName: d3.select(e.target).attr("name")
      });
    };
    ...
    PcpView.prototype.showExpressionPointText = function(e) {
      ...
      e.stopPropagation();
      d = e.target.__data__;
      return this.pcp.append("text").attr("class", "expression-text").attr("x", this.x(d.condition)).attr("y", this.y(d.geneExpression)).attr("text-anchor", "middle").attr("dy", "-.8em").text(d3.round(d.geneExpression, 2));
    };

    PcpView.prototype.showCurrentGeneName = function() {
      var currentGene, currentGeneName;
      ...
      currentGeneName = this.model.get("currentGeneName");
      if (currentGeneName != null) {
        currentGene = this.pcp.select("[name=" + currentGeneName + "]");
        ...
        return this.pcp.append("text").attr("class", "title").attr("x", this.width / 2).attr("y", -40).attr("text-anchor", "middle").text(this.model.getGeneSymbol(currentGeneName));
      } else {
        return this.pcp.selectAll("text.title").filter(".clicked").style("display", "block");
      }
    };

    PcpView.prototype.showClickedGeneName = function() {
      var clickedGene, clickedGeneName, data;
      clickedGeneName = this.model.get("clickedGeneName");
      if (clickedGeneName != null) {
        clickedGene = this.pcp.select("[name=" + clickedGeneName + "]");
        clickedGene.node().parentNode.appendChild(clickedGene.node());
        this.removeClickedGeneName();
        ...
        this.pcp.selectAll(".expression-text").data(data.filter(this.filterTextDatapoints)).enter().append("text").attr("class", "expression-text").attr("x", this.line.x()).attr("y", this.line.y()).attr("text-anchor", "middle").attr("dy", "-.8em").text(function(d, i) {
          return d3.round(d.geneExpression, 2);
        });
        return this.pcp.select("text.title").classed("clicked", 1).text(this.model.getGeneSymbol(clickedGeneName));
      } else {
        return this.removeClickedGeneName();
      }
    };
    ...
    PcpView.prototype.filterTextDatapoints = function(item, i, arr) {
      ...
      return (i === 0 && _.isFinite(item.geneExpression)) || ((i === arr.length - 1) && _.isFinite(item.geneExpression)) || (arr[i - 1].geneExpression === null && _.isFinite(item.geneExpression)) || (arr[i + 1].geneExpression === null && _.isFinite(item.geneExpression)) || (_.isFinite(item.geneExpression) && item.geneExpression > arr[i - 1].geneExpression && item.geneExpression > arr[i + 1].geneExpression && (Math.abs(item.geneExpression - arr[i - 1].geneExpression) > significantDifference || Math.abs(item.geneExpression - arr[i + 1].geneExpression) > significantDifference)) || (_.isFinite(item.geneExpression) && item.geneExpression < arr[i - 1].geneExpression && item.geneExpression < arr[i + 1].geneExpression && (Math.abs(item.geneExpression - arr[i - 1].geneExpression) > significantDifference || Math.abs(item.geneExpression - arr[i + 1].geneExpression) > significantDifference));
    };

    PcpView.prototype.removeClickedGeneName = function() {
    ...
    PcpView.prototype.showCurrentCluster = function() {
      ...
      if (currentCluster) {
        ...
        return this.addGeneExpressionLines();
      }
    };

    PcpView.prototype.showCurrentTag = function() {
      var currentTag, taggedGeneNames;
      currentTag = this.model.get("currentTag");
      d3.select("#pcp").style("top", 0);
      scroll(0, 0);
      if (currentTag) {
        taggedGeneNames = _(d3.selectAll("#heatmap .row")).chain().first().filter(function(d) {
          return d.style.display !== "none";
        }).map(function(d) {
          return d3.select(d).attr("name");
        }).value();
        return this.pcp.selectAll(".expression-line").attr("stroke", "#999").filter(function(d, i) {
          return _.include(taggedGeneNames, d3.select(this).attr("name"));
        }).attr("stroke", function(d, i) {
